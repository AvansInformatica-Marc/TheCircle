...
public abstract class Camera2Base implements GetAacData, GetVideoData, GetMicrophoneData {
  ...
  public Camera2Base(@NonNull Context context) {
  ...
  public void startStreamAndRecord(String path, RecordController.Listener listener) throws IOException {
    startStream();
    recordController.startRecord(path, listener);
  }

  public void startStreamAndRecord(String path) throws IOException {
    startStreamAndRecord(path, null);
  }
  ...
  private void startEncoders() {
    ...
    if (!cameraManager.isRunning() && videoEncoder.getWidth() != previewWidth || videoEncoder.getHeight() != previewHeight) {
      cameraManager.openLastCamera();
    }
    onPreview = true;
  }
  ...
  //cache control
  public abstract FrameStatistics getFrameStatistics();
